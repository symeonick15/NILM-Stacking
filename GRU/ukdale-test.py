...
# disaggregator.import_model("GRU-MultiHouse-{}-{}epochs.h5".format(meter_key,
#                                                                       epochs))
for i in range(totalCheckpoints):
    print("CHECKPOINT {}".format(epochs))
    disaggregator.train_across_buildings(mainsList, meterList, epochs=epochsPerCheckpoint, sample_period=sample_period, batch_size=128)
    epochs += epochsPerCheckpoint
    disaggregator.export_model("GRU-MultiHouse-{}-{}epochs.h5".format(meter_key,
                                                        epochs))
end = time.time()
...
# chunkY = next(test_elec[meter_key].power_series(sample_period = sample_period))
