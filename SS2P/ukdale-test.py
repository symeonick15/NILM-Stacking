...
disaggregator = ShortSeq2PointDisaggregator(window_size=50)
...
# disaggregator.import_model("SS2P-MultiHouse-{}-{}epochs.h5".format(meter_key,
#                                                                       epochs))
for i in range(totalCheckpoints):
    print("CHECKPOINT {}".format(epochs))
    disaggregator.train_across_buildings(mainsList, meterList, epochs=epochsPerCheckpoint, sample_period=sample_period, batch_size=512)
    epochs += epochsPerCheckpoint
    disaggregator.export_model("SS2P-MultiHouse-{}-{}epochs.h5".format(meter_key,
                                                                      epochs))
end = time.time()
