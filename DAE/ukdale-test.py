...
# -- Read dataset info for base models from file (sample file included in project folder)
...
disaggregator = DAEDisaggregator(150)
...
# disaggregator.import_model("DAE-MultiHouse-{}-{}epochs.h5".format(meter_key,
#                                                                       epochs))
for i in range(totalCheckpoints):
    print("CHECKPOINT {}".format(epochs))
    disaggregator.train_across_buildings(mainsList, meterList, batch_size=16, epochs=epochsPerCheckpoint, sample_period=sample_period)
    epochs += epochsPerCheckpoint
    disaggregator.export_model("DAE-MultiHouse-{}-{}epochs.h5".format(meter_key,
                                                                      epochs))
...
# Below functions are used for plotting (1st = NILMTK (downsampled), 2nd = normal)

# # res_elec[meter_key].plot()
# # test_elec2[meter_key].plot()
# # plt.show()
#
# chunkY = next(test_elec2[meter_key].power_series(sample_period=sample_period))
